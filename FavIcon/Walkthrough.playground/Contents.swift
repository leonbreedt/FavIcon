...
import FavIcon
//: We'll use `downloadPreferred(url:completion:)` first, which tries to download an icon
...
try FavIcons.downloadPreferred(url: "https://apple.com", width: 32, height: 32) { result in
...
//: If you want to download all detected icons (maybe you like collecting them), call `downloadAll(url:completion:)`.
