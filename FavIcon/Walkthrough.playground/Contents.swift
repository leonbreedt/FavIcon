...
try FavIcon.downloadPreferred(url: "https://apple.com") { result in
...
try FavIcon.downloadAll(url: "https://microsoft.com") { results in
...
FavIcon.scan(url: URL(string: "https://google.com")!) { icons in
    for icon in icons {
...
//: If one is to your liking, you can download it using `download(url:completion:)`.
        FavIcon.download(icons: [icon]) { results in
